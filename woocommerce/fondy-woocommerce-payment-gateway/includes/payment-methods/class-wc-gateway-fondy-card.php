    ...
    public function init_form_fields()
    {
        $this->form_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'label' => __('Enable Flitt Gateway', 'fondy-woocommerce-payment-gateway'),
                'default' => 'no',
            ...
            'title' => [
                ...
                'default' => __('Flitt Cards, Apple/Google Pay', 'fondy-woocommerce-payment-gateway'),
                'desc_tip' => true,
            ],
            'description' => [
                'title' => __('Description:', 'fondy-woocommerce-payment-gateway'),
                'type' => 'textarea',
                'default' => __('Pay securely by Credit/Debit Card or by Apple/Google Pay with Flitt.', 'fondy-woocommerce-payment-gateway'),
            ...
            'merchant_id' => [
                ...
                'type' => 'text',
                'description' => __('Given to Merchant by Flitt', 'fondy-woocommerce-payment-gateway'),
                'desc_tip' => true
            ],
            'secret_key' => [
        ...
        try {
            ...
            switch ($reverse->reverse_status) {
                ...
                case 'processing':
                    ...
                    $order->add_order_note(sprintf(__('Refund Flitt status: %1$s', 'fondy-woocommerce-payment-gateway'), $reverse->reverse_status));
                    return true;
                case 'declined':
                    $noteText = sprintf(__('Refund Flitt status: %1$s', 'fondy-woocommerce-payment-gateway'), $reverse->reverse_status);
                    $order->add_order_note($noteText);
                    throw new Exception($noteText);
                default:
                    $noteText = sprintf(__('Refund Flitt status: %1$s', 'fondy-woocommerce-payment-gateway'), 'Unknown');
                    $order->add_order_note($noteText);
                    throw new Exception($noteText);
            }
        } catch (Exception $e) {
