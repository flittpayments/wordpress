...
abstract class WC_Fondy_Payment_Gateway extends WC_Payment_Gateway
{
    ...
    const META_NAME_FONDY_ORDER_ID = '_flitt_order_id';
...
//        add_action('woocommerce_before_thankyou', [$this, '']);
        ...
        try {
            ...
            switch ($requestBody['order_status']) {
                ...
                case self::ORDER_DECLINED:
                    ...
                    $orderNote = sprintf(__('Transaction ERROR: order %1$s<br/>Flitt ID: %2$s', 'fondy-woocommerce-payment-gateway'), $requestBody['order_status'], $requestBody['payment_id']);
    ...
    public function fondyPaymentComplete($order, $transactionID)
    {
        if (!$order->is_paid()) {
            ...
            $orderNote = sprintf(__('Fondy payment successful.<br/>Flitt ID: %1$s<br/>', 'fondy-woocommerce-payment-gateway'), $transactionID);
