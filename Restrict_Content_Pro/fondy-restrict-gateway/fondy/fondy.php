...
class RCP_Payment_Gateway_Fondy extends RCP_Payment_Gateway
{
    ...
    public function init()
    {
        ...
        $this->supports[] = 'fees';
        $this->supports[] = 'trial';
        $this->api_endpoint = 'https://pay.flitt.com/api/checkout/url/';
    ...
    public function process_webhooks()
    {
        ...
        rcp_log('Starting to process Flitt webhook.');
        ...
        if ($paymentInfo === true) {
            ...
            if (!$member || !$member->ID > 0) {
                rcp_log('Flitt - member ID not found.');
            ...
            if (!$subscription_id) {
                rcp_log('Flitt - no subscription ID for member.');
            ...
            $payment_data = array(
                ...
                'payment_type' => 'Flitt Credit Card',
            ...
            rcp_log(sprintf('Processing Flitt. Payment status: %s', $posted['order_status']));
            ...
            switch (strtolower($posted['order_status'])) :
                case 'approved':
                    ...
                    break;
                case 'declined' :
                    rcp_log('Processing Flitt declined webhook.');
                    $member->cancel();
                    die('payment declined');
                case 'expired' :
                    rcp_log('Processing Flitt expired webhook.');

                    $member->set_status('expired');

                    $member->set_expiration_date(date('Y-m-d H:i:s', strtotime($posted['order_time'])));

                    $member->add_note(__('Subscription expired in Flitt', 'fondy_rcp'));
